...
const CookieConsent = () => {
  ...
  const handleChoice = (accepted) => {
    localStorage.setItem('cookieConsent', accepted ? 'allowed' : 'declined');
    setVisible(false);
  };
    ...
    <StyledWrapper>
      <div className="card">
        ...
        <p className="cookieDescription">
          ...
          <a href="#privacy">Read cookies policies</a>.
        </p>
        <div className="buttonContainer">
          <button className="acceptButton" onClick={() => handleChoice(true)}>Allow</button>
          <button className="declineButton" onClick={() => handleChoice(false)}>Decline</button>
        </div>
      </div>
    </StyledWrapper>
  );
...
const StyledWrapper = styled.div`
  ...
  .card {
    ...
    position: relative;
    overflow: hidden;
    box-shadow: 0px 10px 30px rgba(0, 0, 0, 0.12), 0px 2px 8px rgba(0, 0, 0, 0.06);
